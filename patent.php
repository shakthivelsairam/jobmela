...
@keyframes fadeout {
  ...
  <body>
    ...
    <div class="container" style="width:95%">
      ...
      <div class="registration-form">
          ...
          try {
		    ...
		    $devCount = "select rowid,link_family_family,title,url,dateofpublished,pattern_number,description from patent_presented where link_family_family=?";
		  ...
		  { 	
				...
				if ($rec1!="") $rec=$rec1[0]."^".$rec1[1]."^".$rec1[2]."^".$rec1[3]."^".$rec1[4]."^".$rec1[5]."^".$rec1[6];
				$p=explode("^",$rec);
				//print_r($p);
				?>
		  <span class="removeRow" id="removeRow" name="removeRow">
			...
			<input type="text" name="pTitle" id="pTitle" class="form-control" placeholder="Title"  value="<?php echo $p[2]; ?>" />  
			</div>
			<div class="col-xs-2 small-txt-box">
			...
			<div class="col-xs-1 small-txt-box">
			<label for="inputCity">Patent Number</label>
			<input type="text" name="pPatent" id="pPatent" class="form-control" placeholder="Patent number"  value="<?php echo $p[5]; ?>" />  
			</div>
			<div class="col-xs-3 small-txt-box">
			<label for="inputCity">Description</label>
			<input type="text" name="pDesc" id="pDesc" class="form-control" placeholder="Description"  value="<?php echo $p[6]; ?>" />  
			</div>
		...
		<div class="form-row">
		<div class="col-xs-12 add-btn" style="border:0px solid red;float:right;width:13%">
	...
	function frowNum(rownum)
	{
		...
		var pDatePublish = document.getElementsByName("pDatePublish");
		
		var pPatent= document.getElementsByName("pPatent");
		var pDesc= document.getElementsByName("pDesc");
		pTitle[rownum].style.border="1px solid #ccc"; 
		pURL[rownum].style.border="1px solid #ccc";
		pDatePublish[rownum].style.border="1px solid #ccc";
		...
		if (pPatent[rownum].value=="") { pPatent[rownum].style.border="1px solid red"; isEmpty=1; }
		if (pDesc[rownum].value=="") { pDesc[rownum].style.border="1px solid red"; isEmpty=1; }
  ...
  $(document).ready(function () {
    ...
    $('.btn-next').on('click', function () {
		...
		for (cnt=0;cnt<totRecs;cnt++)
		{
			...
			pURLtxt=pURLtxt+"|"+pURL[cnt].value;
			pDatePublishtxt=pDatePublishtxt+"|"+pDatePublish[cnt].value;
			pPatenttxt=pPatenttxt+"|"+pPatent[cnt].value;
  ...
  function clearPrompts()
{
	$('.singleRow').html(singRow);
		...
		document.getElementsByName("pTitle")[0].value="";
		document.getElementsByName("pURL")[0].value="";
		document.getElementsByName("pDatePublish")[0].value="";
